...
exports.runEmailProviderContractTests = runEmailProviderContractTests;
 ...
 */
function runEmailProviderContractTests(adapterName, makeProvider) {
    describe(`EmailProviderPort contract — ${adapterName}`, () => {
        ...
        it('acepta múltiples destinatarios', async () => {
            ...
            expect(result.accepted).toEqual(expect.arrayContaining(['uno@example.com', 'dos@example.com']));
        });
        ...
        it('lanza EmailValidationError si no hay destinatarios', async () => {
            const provider = makeProvider();
            await expect(provider.send({ ...baseMessage, to: [] })).rejects.toBeInstanceOf(errors_1.EmailValidationError);
        });
        it('lanza EmailValidationError si el asunto está vacío', async () => {
            const provider = makeProvider();
            await expect(provider.send({ ...baseMessage, subject: '' })).rejects.toBeInstanceOf(errors_1.EmailValidationError);
        });
        it('lanza EmailValidationError si una dirección está vacía', async () => {
            const provider = makeProvider();
            await expect(provider.send({ ...baseMessage, to: { email: '' } })).rejects.toBeInstanceOf(errors_1.EmailValidationError);
        });
        it('lanza EmailValidationError si una dirección de cc/bcc está vacía', async () => {
            const provider = makeProvider();
            await expect(provider.send({ ...baseMessage, cc: [{ email: '' }] })).rejects.toBeInstanceOf(errors_1.EmailValidationError);
        });
        it('lanza EmailValidationError si from está vacío', async () => {
            const provider = makeProvider();
            await expect(provider.send({ ...baseMessage, from: { email: '' } })).rejects.toBeInstanceOf(errors_1.EmailValidationError);
        });
        it('lanza EmailValidationError si replyTo está vacío', async () => {
            const provider = makeProvider();
            await expect(provider.send({ ...baseMessage, replyTo: { email: '' } })).rejects.toBeInstanceOf(errors_1.EmailValidationError);
        });
        it('lanza EmailValidationError si el contenido html está vacío', async () => {
            ...
            })).rejects.toBeInstanceOf(errors_1.EmailValidationError);
        });
        it('lanza EmailValidationError si el contenido text está vacío', async () => {
            const provider = makeProvider();
            await expect(provider.send({ ...baseMessage, content: { kind: 'text', text: '' } })).rejects.toBeInstanceOf(errors_1.EmailValidationError);
        });
        ...
        it('lanza EmailPermanentRejectionError si todos los destinatarios son rechazados', async () => {
            const provider = makeProvider();
            await expect(provider.send({ ...baseMessage, to: { email: 'no-es-un-email' } })).rejects.toBeInstanceOf(errors_1.EmailPermanentRejectionError);
        });
        ...
        it('todos los errores lanzados son instancias de EmailError', async () => {
            const provider = makeProvider();
            await expect(provider.send({ ...baseMessage, to: [] })).rejects.toBeInstanceOf(errors_1.EmailError);
        });
    });
}
